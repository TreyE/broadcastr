...
module Broadcastr
  class AmqpEventWorker
    ...
    def self.run
      Rails.application.config.broadcastr.register_amqp_workers!
      pid_file_location = File.join(File.expand_path(Rails.root), "pids", "sneakers.pid")
      worker_classes = Rails.application.config.broadcastr.sneakers_worker_classes
      ensure_messaging_exchanges
      Sneakers.configure(
        :workers => worker_classes.length,
        :amqp => Rails.application.config.broadcastr.broker_uri,
